...
class  WebRtcAudioRecord {
      ...
      while (keepAlive) {
        ...
        if (bytesRead == byteBuffer.capacity()) {
          try{
            ...
            keepAlive = false;
            Logging.e(TAG,"nativeDataIsRecorded failed: " + e.getLocalizedMessage());
          }
        } else {
  ...
  private boolean startRecording() {
    ...
    if (audioRecord.getRecordingState() != AudioRecord.RECORDSTATE_RECORDING) {
      ...
      if (callSession != null){
        callSession.callState = CallSession.STATUS_RECORD_FAIL;
        CallApi.sendCallRecordErrorBroadcast(callSession);
      }else{
        Logging.e(TAG,"AudioRecord.sendCallRecordErrorBroadcast failed");
      }
  ...
  private static void assertTrue(boolean condition) {
    if (!condition) {
      ...
      Log.i(TAG, "Expected condition to be true");
    }
  }
